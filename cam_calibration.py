...
def move_point(dx, dy=0):

	global loc
	global loc2

	if 0 <= current_point_index < len(loc):
		x, y = loc2[current_point_index]
		x += dx
		y += dy
		loc[current_point_index] = (y, x)
		loc2[current_point_index] = (x, y)
		plt.clf()  # Clear the plot
		plt.imshow(img)  # Display the image again
		plt.plot(*zip(*loc2), 'rx')  # Plot all the selected points
		plt.draw()
...
def manual_crosspoints(file):
	...
	img_resized=cv2.resize(img, None, fx=resize, fy=resize, interpolation=cv2.INTER_CUBIC)
...
def manual_crosspoints(img):
    ...
    plt.connect('button_press_event', onclick)
    plt.connect('key_press_event', onkey)
    plt.title('Select the crosspoints pulentos')
    plt.show()
	'''

def find_crosspoints_m(file,board_r,board_c):

	manual_crosspoints(file)
	adjusted_points = loc
	resize=0.75

	points=[(int(x / resize), int(y / resize)) for x, y in adjusted_points]
